...
2. Determine the copy format by analyzing BOTH the user's request AND the actual tone and content of the text they provide.
  ...
  if (parts.fixGrammar) {
    instructions.push("9. Audit the existing copy for targeted, surgical surface-level errors ONLY. A valid grammar fix is ONE of: a wrong-case word (e.g. 'submit Form' → 'Submit Form'), a stray punctuation mark (e.g. trailing '!' → remove it), passive blame phrasing ('You entered' → 'Enter'), or a standalone filler word ('simply', 'easily', 'great news') that can be deleted without rewriting the sentence. HARD RULES — violating any of these means the fix must be dropped: (a) Never use a variant's output text as the 'corrected' value — if the corrected text matches any variant, it is NOT a grammar fix. (b) Never break a variant rewrite into sub-fixes — grammar fixes must be independent of what the variants already address. (c) Never flag the entire original sentence as a fix — fixes target a specific word or punctuation mark, not the whole phrase. (d) If fixing a phrase requires changing more than ~25% of its characters, it belongs in variants, not here. One entry per unique original text. If a word should be removed, set corrected to \"\". Never use \"Removed.\" as a value. The 'rule' field: short label 5–8 words max. Return as { original, corrected, rule }. Omit if approved=true or needsClarification=true.");
  }
