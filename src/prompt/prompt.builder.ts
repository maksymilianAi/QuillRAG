...
2. Determine the copy format by analyzing BOTH the user's request AND the actual tone and content of the text they provide.
  ...
  if (parts.fixGrammar) {
    instructions.push("9. Grammar audit — only applies when the user provided existing copy to review or improve (i.e. the 'original' field is populated). If the user is writing from scratch with no existing copy, return fixes: [] immediately.\n\n   Check for actual rule violations only — three types, nothing else:\n   A) Wrong capitalization: a word that violates the Book Style or sentence style rules defined in the system prompt. Example: 'submit Form' → 'Submit Form'.\n   B) Prohibited punctuation: a trailing '!' or '…' in functional copy. Set corrected to \"\" to remove it.\n   C) Passive blame phrasing: 'You entered' → 'Enter', 'You selected' → 'Select'.\n\n   Everything else — redundant words, filler, style quality, copy improvements — belongs in reasoning or variants, NOT here. If you cannot point to a specific rule from A, B, or C, return fixes: [].\n\n   HARD DISQUALIFIERS — drop the fix if: (a) corrected value matches any variant output; (b) the fix covers more than one violation in one entry; (c) the fix rewrites more than ~25% of the phrase's characters; (d) original value is a complete sentence (ends with . ? !) or is the entire original text; (e) fix duplicates a change already in a variant.\n\n   One entry per unique original string. To remove a punctuation mark, set corrected to \"\". The 'rule' field: 5–8 words max (e.g. \"Book Style capitalisation\", \"No exclamation marks\", \"Passive blame phrasing\"). Return as { original, corrected, rule }. Omit if approved=true or needsClarification=true.");
  }
