...
2. Determine the copy format by analyzing BOTH the user's request AND the actual tone and content of the text they provide.
  ...
  if (parts.fixGrammar) {
    instructions.push("9. Audit the existing copy for targeted, surgical surface-level errors only: wrong capitalization on a specific word (e.g. 'submit Form' → 'Submit Form'), a missing or extra punctuation mark, passive blame phrasing ('You entered' → 'Enter'), filler/marketing words ('simply', 'easily', 'great news'). A fix must be a small targeted correction — if correcting requires rewriting more than ~30% of the phrase, it belongs in 'variants', NOT here. Do NOT repeat changes that are already expressed in the variants. Do NOT include the overall copywriting rewrite as a fix. One entry per unique original text — if the same phrase has multiple violations, combine into a single entry. If a word or phrase should be removed entirely, set corrected to \"\". Never use \"Removed.\" as a value. The 'rule' field must be a short label (5–8 words max) — e.g. \"Passive blame phrasing\", \"No exclamation marks\". Return as { original, corrected, rule }. Omit if approved=true or needsClarification=true.");
  }
