...
2. Determine the copy format by analyzing BOTH the user's request AND the actual tone and content of the text they provide.
  ...
  if (parts.fixGrammar) {
    instructions.push("9. Grammar audit — only applies when the user provided existing copy to review or improve (i.e. the 'original' field is populated). If the user is writing from scratch with no existing copy, return fixes: [] immediately — there is nothing to audit.\n\n   When original copy exists: scan it for surface-level violations only. A valid fix targets ONE of: (i) a wrong-case word ('submit Form' → 'Submit Form'), (ii) a stray punctuation mark (trailing '!' → remove), (iii) passive blame phrasing ('You entered' → 'Enter'), (iv) a word that appears verbatim in the banned marketing list: powerful, seamlessly, robust, comprehensive, streamline, effortlessly, simply, easily, just, unlock, empower, elevate, amazing, delightful, friendly reminder, great news, don't forget — and only if it can be deleted without restructuring the sentence. Do NOT flag any word as unnecessary or redundant unless it appears verbatim in that list.\n\n   HARD DISQUALIFIERS — if any apply, do not include the fix: (a) The corrected value matches the output of any variant. (b) The fix covers more than one independent violation in the same entry — split or drop. (c) The fix rewrites more than ~25% of the phrase's characters. (d) The original value is a complete sentence (ends with . ? !) or spans the entire original text — never flag a whole sentence for deletion or rewrite; only flag a word or short phrase within it. (e) The fix conveys the same change already expressed by a variant. (f) The word is absent from the verbatim banned list — do NOT infer that a word is "similar to" or "in the spirit of" a banned word; only exact string matches qualify. Words like "automatically", "immediately", "instantly", "quickly" are functional/technical and must never be flagged.\n\n   One entry per unique original string. To remove a word, set corrected to \"\". Never use \"Removed.\" as a value. The 'rule' field: 5–8 words max (e.g. \"Filler word\", \"No exclamation marks\", \"Passive blame phrasing\"). Return as { original, corrected, rule }. Omit if approved=true or needsClarification=true.");
  }
