...
2. Determine the copy format by analyzing BOTH the user's request AND the actual tone and content of the text they provide.
  ...
  if (parts.fixGrammar) {
    instructions.push("9. Audit the existing copy for surface-level grammar and punctuation errors only: wrong capitalization style (e.g. sentence style used where Book Style is required, or vice versa), missing or incorrect punctuation (period at end of tooltip/error, exclamation mark that should be removed), passive blame phrasing ('You entered' → 'Enter'), filler/marketing words ('simply', 'easily', 'great news'). Do NOT include copywriting direction changes here — those belong in 'reasoning'. One entry per unique original text — if the same phrase has multiple violations, combine all broken rules into a single 'rule' string. If a phrase should be removed entirely, set corrected to an empty string \"\". Never use \"Removed.\" or similar prose as the corrected value. The 'rule' field must be a short label (5–8 words max) citing only the specific rule broken — e.g. \"Passive blame phrasing\", \"No exclamation marks\", \"Filler phrase\". Do not write explanations or rationale in 'rule'. Return as { original, corrected, rule }. Omit if approved=true or needsClarification=true.");
  }
